...
applicable section passes. The examples use absolute paths and the built CLI.
...
- the first changed sync is one atomic replacement;
- a failed replacement leaves the old complete view intact;
...
SQLite open/prepare/query, and key/value-read adapters with the physical source instance, operation,
table/key class, and `catalog-metadata` versus `conversation-payload` classification. Install
...
Windows skips only POSIX mode-bit assertions; it still verifies uniqueness, cleanup, no collision,
and typed failures.
...
current 24.x LTS, and current 26.x stable.
...
The smoke must also confirm backup manifests report the current package producer version rather than
the historical hard-coded `0.9.2`.
...
Freeze the affected commands' released stream and exit behavior from the v0.17 tag before changing
their error objects. In particular, existing `--json` list/show/search/export invalid-backup paths
and migrate-session fatal paths that emit their JSON object on stdout continue doing so in this
corrective release; non-JSON paths and existing shared error-handler paths retain their own released
stream. Existing exit categories `1`–`4` keep their released meanings (and unaffected command-specific
codes remain untouched). New ambiguity, scope, capability, and cleanup failures map to the nearest
existing category without moving the stream.

Every JSON error object may gain only additive stable `code`, safe `details`, and remedy fields and
must expose no stack, content, or raw locator. Explicitly reported partial success remains exit `0`
in the successful envelope. The test compares stdout bytes, stderr bytes, object shape, and exit code
to the locked tagged baseline for every affected command. Moving all JSON fatal errors to stderr is
a separate compatibility transition and is not an acceptance condition for feature 016.
...
the safe v0.16 Composer-only incremental-upgrade path.

## 15. Privacy-safe maintainer verification

After automated gates pass, a maintainer performs a final manual check using only data they own and
are authorized to inspect. Exercise the built release candidate against maintainer-owned real Cursor
live data, a real JSON/Markdown export, a newly created/read backup archive, and an explicitly chosen
custom data path. Do not use contributor, employer, customer, shared-machine, or CI-user history.
