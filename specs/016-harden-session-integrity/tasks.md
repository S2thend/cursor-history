...
- [ ] T114 After all repository artifacts are frozen, verify that the candidate revision and tracked tree remained byte-identical through T113; verify that the frozen tracked tree, package inputs, and recurring CI contain no retained third-party implementation, real-data-derived evidence, or generated downstream database/archive; verify that reachable history contains no full upstream third-party source, raw/private Cursor data, or generated downstream database/archive; and record the maintainer-approved narrow exception for the already-removed locally derived compatibility harness in prior feature-branch commits, which requires no history rewrite and authorizes no new historical exception. Then run `npm ci`, typecheck, lint, format check, standard tests—including executable v0.16 fixture regeneration/hash/sensitive-scan/poison and preflight-policy suites—and build through the required scripts without modifying tracked inputs in `package.json`, fixing any sanitization or validation failure only by returning to the owning task and then rerunning T112–T114 from the start
...
- [X] T145 Project leading, middle, and trailing Store-only turns onto the resolved active branch, rebuild their parent chain without admitting Store sidechains, mirror the resolved IDs into legacy `activeBranchBubbleIds` and additive `activeBranchMessageIds`, and prove both preferred backbones remain consumable by the unchanged v0.16 vibe-history adapter in `src/core/store-stack/merge.ts`, `tests/unit/store-stack-merge.test.ts`, and `tests/compatibility/v016-composer-upgrade.test.ts`
...
- **Phase 11 — Post-audit corrective completion**: T116–T147 may proceed by independent file group
  ...
  T114 revalidates the frozen tree/package/CI, excludes full upstream source, raw/private data, and
  generated downstream databases/archives from reachable history, and does not require rewriting
  the accepted prior harness commits.
