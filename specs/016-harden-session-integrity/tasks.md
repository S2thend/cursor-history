...
- [X] T138 [P] Apply case-insensitive logical matching only to canonical UUID syntax while preserving every source-native public spelling and exact physical SQLite key/path locator; make Composer spelling authoritative for Composer-backed views, keep non-UUID identifiers byte-sensitive, reconcile equivalent case variants, return typed ambiguity for divergent variants, and bind scoped direct-ID and migration reads without case-folded physical SQL in `src/core/session-id.ts`, `src/core/composer-sqlite.ts`, `src/core/session-catalog.ts`, `src/core/storage.ts`, `src/core/store-stack/discover.ts`, `src/core/migrate.ts`, `src/cli/commands/session-lookup.ts`, `tests/integration/composer-session-id-case-sensitivity.test.ts`, `tests/integration/migrate-session-scope.test.ts`, `tests/unit/composer-sqlite.test.ts`, `tests/unit/session-catalog.test.ts`, and `tests/unit/storage-context.test.ts`
...
- [X] T140 Reconcile Composer global payloads by exact physical spelling before workspace membership rows, suppress duplicate Store projections, retain the selected compatible workspace title/session timestamp only when optional global metadata is absent, keep global bubbles as the sole payload authority, and lock live/backup/global-only/one-or-many-case-variant/metadata-free/session-fallback behavior in `src/core/storage.ts`, `tests/e2e/global-composer-catch-all.test.ts`, `tests/integration/composer-source-arbitration.test.ts`, `tests/integration/composer-session-id-case-sensitivity.test.ts`, and `tests/unit/storage.test.ts`
...
- [X] T142 Bind migration's logical canonical UUID separately from the exact source-native workspace and global SQLite spellings; preserve the workspace/Composer public spelling, read and mutate only the bound exact global keys, reject multiple case-only global occurrences before writes, and keep arbitrary and compact 32-hex non-UUID Store identifiers byte-sensitive in `src/core/migrate.ts`, `src/core/session-id.ts`, `tests/integration/migrate-session-scope.test.ts`, and `tests/integration/composer-session-id-case-sensitivity.test.ts`
...
- [X] T144 Resolve a lower-case pointer-only workspace membership to a sole upper-case unstamped global Composer carrier by canonical UUID identity while keeping the global carrier's exact native public/physical spelling; cover live/custom workspace discovery, scoped list/get/search, and nonmatching physical reads in `src/core/storage.ts` and `tests/integration/composer-session-id-case-sensitivity.test.ts`
...
- [X] T147 Reconcile FR-083–FR-089, SC-021–SC-026, phase ranges, task/checklist traceability, logical-versus-physical migration semantics, v0.16 collation/active-branch compatibility, pointer-only membership, package-smoke topology, and the settled backup envelope decision (`manifest.version: "1.0.0"`, inventory `schemaVersion: 1`) across `specs/016-harden-session-integrity/` and shipped `docs/`
...
| FR-084: canonical UUID logical selection and exact physical migration keys | T142, T147, T110, T113–T115 |
...
| FR-087: pointer-only workspace association with opposite-case sole global carrier | T144, T147, T110, T113–T115 |
...
| SC-022: UUID spelling and exact physical migration keys round-trip safely | T142, T147, T110, T113–T115 |
| SC-023: migration reads no off-scope payload and refuses atomically | T143, T147, T110, T114–T115 |
| SC-024: pointer-only opposite-case global carrier resolves once | T144, T147, T110, T113–T115 |
