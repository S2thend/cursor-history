...
- [ ] T114 After all repository artifacts are frozen, verify that the current tree contains no copied or derived third-party consumer implementation and that neither the current tree nor reachable branch history contains an upstream vibe-history source file, private Cursor input, or generated downstream database/archive; the maintainer explicitly accepts the deleted compact derived test-harness revisions remaining in feature-branch ancestry, so those revisions alone do not require a history rewrite. Then run `npm ci`, typecheck, lint, format check, standard tests—including executable v0.16 fixture regeneration/hash/sensitive-scan/poison and preflight-policy suites—and build through the required scripts without modifying tracked inputs in `package.json`, fixing any sanitization or validation failure only by returning to the owning task and then rerunning T112–T114 from the start
...
- **Phase 11 — Post-audit corrective completion**: T116–T147 may proceed by independent file group
  ...
  Any failure returns to its owning T116–T147 task and restarts T110–T115. T132 supplies the
  current-tree cleanup; T114 separately verifies the maintainer-approved upstream-source,
  private-input, and generated-artifact history boundary.
