...
- [ ] T113 Before freezing artifacts, create a fresh owner-private certification root, verify the maintainer-approved source-archive digest without retaining it in aggregate evidence, safely extract the archive, and require an exact private whole-tree path/type/mode/size/content manifest match from before any data access to a `finally` path after every handled outcome; an uncatchable termination invalidates the run, and its owner-private residue MUST be refused for reuse and deleted before retry. Bind the run before dynamic import or source access to the candidate revision and single checksum-addressed pre-freeze packed candidate, official `v0.16.0` revision/tree/distribution digest, ARR revision/source-blob inventory, harness manifest, runtime production-dependency tree, and exact policy version. Perform a one-time maintainer-authorized full-corpus differential outside the repository between that oracle and candidate over the same Composer-only source: compare every pre-existing library session/message/code-block/tool value and own-property/null/omission shape separately from allowlisted additive fields and documented versioned exceptions—including classifying only a missing/null/empty legacy message `id` becoming its exact v0.16 ordinal fallback key as the FR-006 identity-property exception—with no sampling, record cap, time budget, or early-success exit; run an independent exhaustive all-candidate association pass even if its straightforward implementation is quadratic. Then run two separately reported, mandatory lanes through the pinned unchanged vibe-history adapter and its real sync policy/SQLite persistence. Lane A uses the entire authorized real Composer-only corpus with Store provably empty: import v0.16, repeat with zero writes, apply the candidate, compare every non-excepted durable old session/message/code-block/tool value and every key, relationship, and binding, separately count each allowed FR-024/FR-036 predicate-guarded scalar correction, and repeat the candidate with zero writes. Lane B uses a wholly fictional Composer-plus-Store fixture with the same unchanged consumer: perform complete replacement, force a late transaction failure, reopen to prove exact rollback, retry successfully, and repeat with zero writes. Lane B MUST NOT copy, transform, hash into, or otherwise derive any ID, path, title, content, timestamp, ordering, or tool value from Lane A, and its result MUST NOT be reported as a Store transition observed in the real corpus. Both lanes MUST pass. Raw source, outputs, IDs, hashes, paths, titles, content, timestamps, private manifests/digests, and downstream databases MUST remain in owner-private temporary storage and be deleted after certification; retain only non-private code/artifact bindings and aggregate pass/fail/category counts outside the repository. Apart from the required ephemeral owner-private archive/tree integrity digests, which MUST be deleted and MUST NOT enter aggregate evidence, real data MAY identify structural coverage gaps but MUST NOT be transformed, copied, redacted, hashed into, or otherwise used as input to committed fixtures or Lane B: reproduce each applicable gap manually with fixed fictional values in the wholly synthetic generator, pass regeneration/hash/sensitive-scan/poison tests, rerun the new regression, and then restart T112–T113. After that differential and the preliminary full required validation both pass, create the frozen item-level FR-001–FR-089/SC-001–SC-026 traceability, Compatibility Matrix v1 result map, and compatibility disposition/evidence for 100% of feature-016 public returned-value changes, plus contract, quickstart, runtime, and constitution gates with no unresolved exception, allowing only this planned checklist write before T114 in `specs/016-harden-session-integrity/checklists/implementation.md`
- [ ] T114 After all repository artifacts are frozen, verify that the frozen tracked tree, package inputs, and recurring CI contain no retained third-party implementation or generated downstream database/archive; verify that reachable history contains no full upstream third-party source, raw/private Cursor data, or generated downstream database/archive; and record the maintainer-approved narrow exception for the already-removed locally derived compatibility harness in prior feature-branch commits, which requires no history rewrite and authorizes no new historical exception. Then run `npm ci`, typecheck, lint, format check, standard tests—including executable v0.16 fixture regeneration/hash/sensitive-scan/poison and preflight-policy suites—and build through the required scripts without modifying tracked inputs in `package.json`, fixing any sanitization or validation failure only by returning to the owning task and then rerunning T112–T114 from the start
